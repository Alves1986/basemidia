...
export default async function handler(
  ...
  const body = (await webRequest.json().catch(() => null)) as { contextCopy?: string, contextStrategy?: string };
  if (!body?.contextCopy || !body?.contextStrategy) {
    return sendWebResponse(jsonResponse({ error: "Faltou a estratégia ou a copy de contexto." }, { status: 400 }), response);
  }
