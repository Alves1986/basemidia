    ...
    <main className="site-shell">
      <nav className="topbar">
        ...
        <div className={`nav-links ${menuOpen ? "is-open" : ""}`}>
          ...
          <button className="nav-link-button" onClick={openDiagnostic}>Diagnóstico</button>
          <button className="nav-cta" onClick={openDiagnostic}>Agendar conversa <ArrowUpRight size={15} /></button>
        </div>
      ...
      <aside className="progress-rail" aria-label="Etapas do diagnóstico">
        ...
        <button onClick={openDiagnostic}><span>03</span><i /><b>AÇÃO</b></button>
      </aside>
      ...
      <section id="top" className="hero-section">
        <div className="hero-copy">
          ...
          <div className="hero-actions">
            <button className="primary-cta" onClick={openDiagnostic}>Quero meu diagnóstico gratuito <ArrowUpRight size={18} /></button>
      ...
      {isFormOpen && <div id="diagnostico" className="modal-backdrop" role="presentation" onMouseDown={closeDiagnostic}>
        <div className="modal-dialog" role="dialog" aria-modal="true" aria-labelledby="diagnostico-title" onMouseDown={(event) => event.stopPropagation()}>
          <button className="modal-close" aria-label="Fechar formulário" onClick={closeDiagnostic}><X size={21} /></button>
          {submitted ? <div className="success-state modal-success"><div className="success-icon"><Check size={28} /></div><div className="section-index">/ RECEBIDO</div><h3>Seu cenário chegou até a gente.</h3><p>Vamos analisar suas respostas e entrar em contato em até 1 dia útil para combinar o próximo passo.</p><button className="primary-cta" onClick={() => { setSubmitted(false); closeDiagnostic(); }}>Fechar <X size={17} /></button></div> : <div className="modal-form-content"><div className="modal-heading"><div className="section-index">/ DIAGNÓSTICO GRATUITO</div><h2 id="diagnostico-title">Conte o cenário.<br /><span>A gente organiza o próximo movimento.</span></h2><p>Preencha em menos de 2 minutos. Suas respostas tornam a conversa mais objetiva desde o primeiro contato.</p><div className="form-benefits"><div><CircleCheck size={18} /><span>Sem pitch genérico</span></div><div><CircleCheck size={18} /><span>Leitura do seu contexto</span></div><div><CircleCheck size={18} /><span>Próximos passos claros</span></div></div></div><div className="form-card"><form onSubmit={submit}><div className="form-top"><span>01 / 01</span><span className="progress-label">FORMULÁRIO RÁPIDO</span><div className="progress-bar"><i /></div></div><div className="form-grid"><label>Seu nome<input required value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="Como podemos te chamar?" /></label><label>WhatsApp<input required type="tel" value={form.whatsapp} onChange={(e) => update("whatsapp", e.target.value)} placeholder="(00) 00000-0000" /></label><label>E-mail<input required type="email" value={form.email} onChange={(e) => update("email", e.target.value)} placeholder="voce@empresa.com" /></label><label>Segmento<select required value={form.segment} onChange={(e) => update("segment", e.target.value)}><option value="">Selecione o segmento</option><option>Negócio local</option><option>E-commerce</option><option>Serviços B2B</option><option>Outro</option></select></label><label>Você já anuncia hoje?<select required value={form.ads} onChange={(e) => update("ads", e.target.value)}><option value="">Escolha uma opção</option><option>Sim, mas sem estratégia clara</option><option>Sim, com frequência</option><option>Ainda não anuncio</option></select></label><label className="full-label">Qual é hoje o maior problema que você quer resolver com tráfego pago?<textarea required value={form.pain} onChange={(e) => update("pain", e.target.value)} placeholder="Ex.: tenho cliques, mas poucos contatos que avançam..." /></label><label className="full-label">O que você quer melhorar nos próximos 90 dias?<input required value={form.goal} onChange={(e) => update("goal", e.target.value)} placeholder="Ex.: gerar demanda com mais previsibilidade" /></label></div><button type="submit" className="primary-cta form-submit">Quero meu diagnóstico gratuito <ArrowUpRight size={18} /></button><p className="privacy-note"><LockKeyhole size={13} /> Seus dados ficam seguros e serão usados apenas para este contato.</p></form></div></div>}
        </div>
      </div>}
    </main>
  );
